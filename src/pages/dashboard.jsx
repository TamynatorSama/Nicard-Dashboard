...
const Dashboard = () => {
  ...
  useEffect(()=>{
    console.log(appState.appData.userData)
    fetch('http://localhost:5001/api/cards/getVendorRequests/fd8044d4-1929-4c62-ab2c-ade74bdfa03a',{
      headers:{
        "authorization": `Bearer ${appState.appData.userData.user_info?.token??""}`
      }
    }).then((res)=>res.json()).then((val)=>{
      if(val.result){
        appState.appDataModifier({userData: appState.appData.userData,
  ...
  var buildListing = appState.appData.vendorData.map((val)=>{
    console.log(val)
    return  <div key={val.id} className="grid grid-cols-8 gap-7  grid-rows-1 bg-stone-200 px-4 py-2 rounded-sm items-center">
    ...
    <p className="text-sm font-semibold">{new Date(val.create_at).toLocaleDateString('en-us', { weekday:"long", year:"numeric", month:"short", day:"numeric"}) }</p>
    <p className="text-sm font-semibold">{val.status.status}</p>
    <p className="text-sm font-semibold">{val.request_type.tag}</p>
  ...
  return (
    <main className="w-full h-svh flex">
      ...
      <div className="py-10 px-10 w-full flex flex-col h-full">
        ...
        {buildListing.length===0?<div className="h-[90%] flex items-center justify-center flex-col">
            <img src={emptyPng} alt=""  className=" w-[25em] h-[80%] opacity-90"/>
            <h1 className="text-3xl font-semibold text-center text-stone-700 max-w-[20em]">No Card Request has been made to this Vendor account</h1>
        </div>:  <div className="flex flex-col gap-5 h-full">
